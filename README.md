...
Android UI 自动化框架，结合 VisionTasker 视觉检测和 LLM 任务规划。

## ✨ 特性

- 🎯 **智能任务规划**: LLM 自动拆解复杂任务为执行步骤
- 📱 **多设备支持**: Android (真机/模拟器)
- 🚀 **双重匹配策略**: XML 快速匹配 + VisionTasker 视觉检测
...
├── external/VisionTasker/ # VisionTasker UI 检测
├── data/                  # 运行数据输出
├── run_project.py         # 启动脚本
...
**流程**: LLM 规划 → 步骤序列 → 逐步执行（XML 优先，失败则 VisionTasker）
...
步骤 2/2: 点击"朋友圈"
  ❌ XML 匹配失败
  🔄 VisionTasker 检测 + LLM 决策 → 点击 (540, 350)
```

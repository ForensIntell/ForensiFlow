...
| 组件 | 技术选型 | 说明 |
|------|---------|------|
| 语言模型 | 任意 OpenAI-compatible 大语言模型 API | 任务规划、决策、脚本生成；Mimo/Momi 只是兼容示例 |
...
```bash
# 任意 OpenAI-compatible 大语言模型 API（必需）
FORENSIFLOW_API_KEY=your-api-key
FORENSIFLOW_API_BASE=https://your-openai-compatible-endpoint/v1
FORENSIFLOW_MODEL=your-model-name

# 兼容别名：MOMI_*/MIMO_*/LLM_* 也可以使用
...
当前 Web UI 已接入 `tools/forensiflow_demo_api.py` 暴露的真实 `/api/*` 接口，用于设备状态、应用列表、任务规划、任务启动、运行状态、证据下载、审计会话和报告生成。后端暂未提供的案件 CRUD、经验库管理和设置写入在前端标注为暂未开放；页面展示只使用后端返回数据或明确的空状态。
...
# 兼容旧配置：MOMI_*/MIMO_*/LLM_*/PAGE_AGENT_MOBILE_* 仍然有效
...
所有大模型调用默认统一使用 OpenAI-compatible LLM 配置：

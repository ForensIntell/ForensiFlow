...
- **老调度器（复用模式）**：XML 文本匹配 + ForensiVision 视觉 fallback，高效复用历史经验
...
| 视觉模型 | ForensiVision (YOLO + CLIP) | UI 元素检测和分类（老调度器 fallback） |
...
- **ForensiVision 模型**（老调度器需要）：下载到 `external/ForensiVision/pt_model/`

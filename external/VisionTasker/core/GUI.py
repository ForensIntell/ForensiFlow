...
# 确保 VisionTasker 根目录在 sys.path 的最前面，这样 layout 模块才能被找到
_visiontasker_root = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))

# 如果 VisionTasker 已经在 sys.path 中但不在位置 0，先移除再添加到位置 0
if _visiontasker_root in sys.path:
    sys.path.remove(_visiontasker_root)
sys.path.insert(0, _visiontasker_root)

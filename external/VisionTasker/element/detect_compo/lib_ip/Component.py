from element.detect_compo.lib_ip.Bbox import Bbox
...
class Component:
    def __init__(self, region, image_shape, sub_category='Unknown'):
        self.id = None  # 组件id 从1开始
        self.region = region  # 连通区域像素点坐标 (x,y)（先col再idx） list [(0, 157), (0, 160), (12, 156), (14, 161), (25, 140), (26, 149...
        self.boundary = self.compo_get_boundary()  # list [ [], [], [], [] ] 上(y最小一行，第2个数小)，下(y最大，第2个数大)，左(交换成为yx 还是第2个数小)，右(交换成为yx 还是第2个数大)
    ...
    def calc_intersection_area(self, compo_b, bias=(0, 0)):
        a = self.put_bbox()  #
        b = compo_b.put_bbox()
        ...
        iou = inter / (self.area + compo_b.area - inter)
        ioa = inter / self.area
        iob = inter / compo_b.area

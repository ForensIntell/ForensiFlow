"""Experimental page perception fusion for ForensiVision and Android XML.

This module is intentionally independent from the forensic execution path. It
normalizes ForensiVision page-perception JSON and simplified Android XML nodes,
...
def parse_bounds(value: Any, *, point_radius: int = 0) -> Optional[Bounds]:
    """Parse supported Android/ForensiVision bounds formats."""
    if value is None:
        return None
...
def extract_forensivision_elements(data: Any, config: Optional[FusionConfig] = None) -> List[NormalizedElement]:
    config = config or FusionConfig()
    elements: List[NormalizedElement] = []
...
def fuse_page_perception(
    *,
    ...
    """Fuse normalized ForensiVision and XML elements into row-like regions."""
    config = config or FusionConfig()
    ...
    for index, group in enumerate(groups, start=1):
        ...
        vision_group = [element for element in group if element.source == "forensivision"]
            ...
            fused_items.append(
                {
                    ...
                    "sources": ["xml"] + (["forensivision"] if best_matches else []),
        ...
        region = {
            ...
            "source_counts": {
                "xml": len(xml_group),
                "forensivision": len(vision_group),
            },
            "texts": {
                "combined": _unique_texts(group),
                "xml": _unique_texts(xml_group),
                "forensivision": _unique_texts(vision_group),
            },
    ...
    return {
        ...
        "stats": {
            ...
            "xml_only_regions": sum(1 for r in regions if r["source_counts"]["forensivision"] == 0),
        },
        "regions": regions,
...
def render_fused_description(regions: Sequence[Dict[str, Any]], config: Optional[FusionConfig] = None) -> str:
    ...
    for region in regions[: config.max_description_regions]:
        ...
        header = (
            ...
            f"sources(xml={counts.get('xml', 0)}, vision={counts.get('forensivision', 0)})"
        )
        lines.append(header)
...
def load_forensivision_json(path: Path, config: Optional[FusionConfig] = None) -> List[NormalizedElement]:
    return extract_forensivision_elements(load_json(path), config=config)
...
def fuse_files(
    *,
    ...
    config = config or FusionConfig()
    vision_elements = load_forensivision_json(vision_json_path, config=config)
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Fuse ForensiVision page JSON and simplified Android XML into a page description."
    )

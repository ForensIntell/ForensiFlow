...
def rescale_bboxes(out_bbox, size):
    device = torch.device("cuda") if torch.cuda.is_available() else torch.device("cpu")  # 保持默认设备设置
    print(f"� 图像处理脚本使用默认设备: {device}")
    img_w, img_h = size
...
def process_img(label_path_dir, img_path, output_root, layout_json_dir,
    ...
    device = torch.device("cuda") if torch.cuda.is_available() else torch.device("cpu")  # 保持默认设备设置（CLIP已在导入时强制CPU）
    print(f"� 主检测函数使用默认设备: {device}")

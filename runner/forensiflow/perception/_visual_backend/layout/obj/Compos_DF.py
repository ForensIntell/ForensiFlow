...
class ComposDF:
    ...
    ******************************
    ...
    def icon_cls_by_clip(self, ori_img, model_cls, preprocess, device, real_labels_dict, labels_for_read, lang='zh'):
        device_type = 'GPU' if str(device) != 'cpu' else 'CPU'
        print(f">>> [DEBUG] 启动 CLIP 图标识别 ({device_type}加速模式)...")
        
        # ---------------------------------------------------------
        # 核心修改：使用GPU加速CLIP推理，提升性能
        # ---------------------------------------------------------
        # 使用传入的device参数（GPU或CPU）
        model_cls = model_cls.to(device)
        # Tokenize labels to the same device as model
        ...
        need_cls_df = need_cls_df.sort_values(['center_row'])
        print(f">>> [DEBUG] 待识别目标数量: {len(need_cls_df)}")
        ...
        for i in range(len(need_cls_df)):
            ...
            if i > 0 and i % 10 == 0:
                ...
                eta = (len(need_cls_df) - i) / speed if speed > 0 else 0
                print(f"⏳ CLIP进度: {i}/{len(need_cls_df)} ({i/len(need_cls_df)*100:.1f}%) - 已用时: {elapsed:.1f}s - 预计剩余: {eta:.1f}s")
            ...
            if curr_id in self.compos_dataframe.index:
                ...
                if 0.8 < cp['width']/cp['height'] < 3 and cp['area'] / (self.img.size / 3) < 0.015:
                    col_cand_id = self.compos_dataframe.loc[(self.compos_dataframe['center_column'] < curr_col + 4) & (curr_col - 15 < self.compos_dataframe['center_column']) 
                                                            ...
                                                            & (self.compos_dataframe['line_split_c'] == cp['line_split_c'])].index
                    if len(col_cand_id) > 0:
                        col_cand_gap = self.compos_dataframe.loc[col_cand_id, 'row_min'] - cp['row_max']
                        gap_th = max(20, cp['height']/3)
                        if min(col_cand_gap) < gap_th:
                            selected_idx = abs(self.compos_dataframe.loc[col_cand_id, 'center_row'] - curr_row).idxmin()
                            self.compos_dataframe.loc[curr_id, 'text_content'] = self.compos_dataframe.loc[selected_idx, 'text_content'] + '_'
                            self.compos_dataframe.loc[curr_id, 'pair_to'] = selected_idx
                            self.compos_dataframe.drop(selected_idx, inplace=True)
                            find_the_text = True

                # --- 横向匹配找回 (保留原逻辑) ---
                if not find_the_text and 0.85 < cp['width'] / cp['height'] < 3 \
                        and cp['area'] / (self.img.size / 3) < 0.025 \
                        and cp['center_row'] > self.img.shape[0] * 0.125 and (len(str(cp['text_content'])) <= 1):
                    row_cand_id = self.compos_dataframe.loc[(cp['row_min'] < self.compos_dataframe['center_row']) & (self.compos_dataframe['center_row'] < cp['row_max'])
                                                            & (self.compos_dataframe['center_column'] >= cp['center_column']) & (self.compos_dataframe['class'] == 'Text')
                                                            & (self.compos_dataframe['id'] != curr_id)
                                                            & (self.compos_dataframe['line_split_c'] == cp['line_split_c'])].index
                    if len(row_cand_id) > 0:
                        ...
                        if len(self.compos_dataframe.loc[(cp['column_max'] <= self.compos_dataframe['center_column']) &
                            ...
                            if 0.975 <= w / h <= 1.025 and cp['area'] / (img_w * img_h) > 0.012 and cp['center_column'] < img_w / 3 and cp['center_row'] < img_h / 3:
                                ...
                                self.compos_dataframe.drop(curr_id, inplace=True)
                            find_the_text = True

                # --- CLIP 识别部分 (核心修改) ---
                if not find_the_text:
                    ...
                    # 裁剪并预处理图片，使用GPU加速
                    cp_img = image_pil.crop((int(x1), int(y1), int(x2), int(y2)))
                    image = preprocess(cp_img).unsqueeze(0).to(device)

                    if cp['area']/(self.img.shape[0] * self.img.shape[1]) < 0.05:
                        with torch.no_grad():
                            try:
                                # 在 GPU 上执行推理，大幅提升性能
                                logits_per_image, logits_per_text = model_cls(image, label)
                                probs = logits_per_image.softmax(dim=-1).cpu().numpy()[0] # 移回CPU处理
                                
                                predicted_class_id = np.argmax(probs) + 1
                                predicted_class = labels_for_read[str(predicted_class_id)]
                                
                                # 补丁修正
                                if np.argmax(probs) + 1 == 148 and w/h >= 4:
                                    predicted_class = '搜索框' if lang == 'zh' else 'Search box'
                                if np.argmax(probs) + 1 in [173, 174] and cp['area']/(self.img.shape[0] * self.img.shape[1]) < 0.0005:
                                    predicted_class = '页面指示器' if lang == 'zh' else 'page indicator'

                                # 写入预测结果
                                if self.compos_dataframe.loc[self.compos_dataframe['id'] == need_cls_df.iloc[i, 0], 'text_content'].values[0] == None or self.compos_dataframe.loc[self.compos_dataframe['id'] == need_cls_df.iloc[i, 0], 'text_content'].values[0] == '':
                                    if not 0.5 <= w/h <= 20:
                                        ...
                                        if cp['area'] / (self.img.shape[0] * self.img.shape[1]) < 0.0015:
                                            self.compos_dataframe.loc[curr_id, 'text_content'] = '可能是' if lang == 'zh' else 'Maybe is a ' + predicted_class + '_'
                                            if sorted(probs)[-2] + sorted(probs)[-1] < 0.3:
                                                self.compos_dataframe.loc[curr_id, 'text_content'] = '图片_' if lang == 'zh' else 'picture_'
                                        else:
                                            self.compos_dataframe.loc[curr_id, 'text_content'] = ('图片，或' if lang == 'zh' else 'a picture or ') + predicted_class + '_'
                                            if 0.975 <= w/h <= 1.025:
                                                ...
                                                if cp['center_column'] < img_w/3 and cp['center_row'] < img_h/3 or cp['center_column'] > img_w*2/3 and cp['center_row'] < img_h*1/3 or img_w/2 - 5 < cp['center_column'] < img_w/2 + 5 and cp['center_row'] < img_h*1/3:
                                                    self.compos_dataframe.loc[curr_id, 'text_content'] = ('头像，或' if lang == 'zh' else 'Avatar or ') + predicted_class + '_'
                                    else:
                                        if predicted_class == '搜索框' or predicted_class == 'Search box':
                                            if cp['area']/(self.img.shape[0] * self.img.shape[1]) < 0.01:
                                            ...
                                            # 横向图标文字匹配
                                            self.compos_dataframe.loc[curr_id, 'text_content'] += predicted_class + ('键' if lang == 'zh' else ' button') + '_'
                                    
                            except Exception as e:
                                print(f"⚠️ CLIP推理时出现异常: {str(e)}")
                                continue
        total_time = time.time() - start_time
        avg_time = total_time / processed_count if processed_count > 0 else 0
        print(f">>> [DEBUG] CLIP 识别完成 - 总耗时: {total_time:.2f}s - 平均每个: {avg_time:.3f}s - 处理速度: {processed_count/total_time:.1f}个/秒")

"""Experimental page perception fusion for visual backend and Android XML.

This module is intentionally independent from the forensic execution path. It
normalizes visual backend page-perception JSON and simplified Android XML nodes,
...
def parse_bounds(value: Any, *, point_radius: int = 0) -> Optional[Bounds]:
    """Parse supported Android/visual backend bounds formats."""
    if value is None:
        return None
...
def extract_visual_elements(data: Any, config: Optional[FusionConfig] = None) -> List[NormalizedElement]:
    config = config or FusionConfig()
    elements: List[NormalizedElement] = []
...
def fuse_page_perception(
    *,
    ...
    """Fuse normalized visual backend and XML elements into row-like regions."""
    config = config or FusionConfig()
    ...
    for index, group in enumerate(groups, start=1):
        ...
        vision_group = [element for element in group if element.source == "visual"]
            ...
            fused_items.append(
                {
                    ...
                    "sources": ["xml"] + (["visual"] if best_matches else []),
        ...
        region = {
            ...
            "source_counts": {
                "xml": len(xml_group),
                "visual": len(vision_group),
            },
            "texts": {
                "combined": _unique_texts(group),
                "xml": _unique_texts(xml_group),
                "visual": _unique_texts(vision_group),
            },
            "xml_nodes": [element.to_summary() for element in xml_group],
            "visual_elements": [element.to_summary() for element in vision_group],
    ...
    return {
        ...
        "stats": {
            "visual_elements": len(vision_elements),
            "xml_nodes": len(xml_elements),
            "regions": len(regions),
            "matched_visual_elements": len(matched_vision_ids),
            "matched_xml_nodes": len(matched_xml_ids),
            "visual_only_regions": sum(1 for r in regions if r["source_counts"]["xml"] == 0),
            "xml_only_regions": sum(1 for r in regions if r["source_counts"]["visual"] == 0),
        },
        "regions": regions,
...
def render_fused_description(regions: Sequence[Dict[str, Any]], config: Optional[FusionConfig] = None) -> str:
    ...
    for region in regions[: config.max_description_regions]:
        ...
        header = (
            ...
            f"sources(xml={counts.get('xml', 0)}, visual={counts.get('visual', 0)})"
        )
        lines.append(header)
...
def load_visual_json(path: Path, config: Optional[FusionConfig] = None) -> List[NormalizedElement]:
    return extract_visual_elements(load_json(path), config=config)
...
def fuse_files(
    *,
    ...
    config = config or FusionConfig()
    vision_elements = load_visual_json(vision_json_path, config=config)
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Fuse visual backend page JSON and simplified Android XML into a page description."
    )
